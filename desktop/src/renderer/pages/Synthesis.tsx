...
interface Clone { id: string; handle: string; name: string; avatar_url?: string; }
interface SynthTurn {
  id: string;
  cloneId: string;
  cloneName: string;
...
const MAX_ROUNDS             = 8;   // max full rounds (all clones speak once)
const MIN_TURNS_FOR_CONCLUDE = 5;   // clones can't signal conclusion before this many total turns

// ─── Prompt Engineering ───────────────────────────────────────────────────────
//
// These prompts go through /brain/chat/stream, so the IdentityLayer already
// injects each clone's persona, epistemic profile, style, and values as the
// system prompt. We only need to provide discussion context + engagement rules.
...
function buildTurnPrompt(
  ...
  // Build history block — clean of any stray markers
  const history = turns
    ...
    <div style={{
      ...
      <style>{`
        ...
        @keyframes particle-drift {
          ...
          50%     { opacity: 0.55; transform: var(--drift); }
        }
        ...
        .clone-card-active  { transition: transform 380ms cubic-bezier(.4,0,.2,1), opacity 380ms, box-shadow 380ms; }
        .clone-card-passive { transition: transform 380ms cubic-bezier(.4,0,.2,1), opacity 380ms, box-shadow 380ms; }
      `}</style>
      ...
      <div style={{ position: "absolute", inset: 0, pointerEvents: "none", zIndex: 0, overflow: "hidden" }}>
        {PARTICLES.map(p => (
          <div key={p.id} style={{
            ...
            // @ts-ignore
            "--drift": `translateX(${p.dx}px) translateY(${p.dy}px)`,
      ...
      <div style={{ position: "relative", zIndex: 1, display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: 13, fontWeight: 600, background: "linear-gradient(110deg,#a78bfa,#38bdf8,#34d399)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
              doppel
            </span>
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: "20px 18px" }} className="synth-scroll">
      ...
      {loading && (
        <div style={{ display: "flex", justifyContent: "center", padding: 24 }}>
          <div style={{ width: 16, height: 16, borderRadius: "50%", border: "2px solid rgba(255,255,255,0.08)", borderTopColor: "rgba(255,255,255,0.40)", animation: "spin 0.8s linear infinite" }} />
        </div>
      )}

      {!loading && allClones.length === 0 && (
        <p style={{ fontSize: 12, color: "rgba(255,255,255,0.30)", textAlign: "center", marginBottom: 20 }}>
          Sign in and create clones to use synthesis.
        </p>
      )}
...
function MessageBubble({ turn, isRight }: { turn: SynthTurn; isRight: boolean }) {
  return (
    <div style={{
      ...
      gap: 9, animation: "msg-in 260ms cubic-bezier(.4,0,.2,1) both",
    }}>
      {/* Avatar */}
      <div style={{
    ...
    <div style={{ display: "flex", flexDirection: isRight ? "row-reverse" : "row", gap: 9 }}>
      ...
      <div style={{ maxWidth: "78%", display: "flex", flexDirection: "column", gap: 3, alignItems: isRight ? "flex-end" : "flex-start" }}>
        <span style={{ fontSize: 9, color: "rgba(255,255,255,0.28)", padding: isRight ? "0 2px 0 0" : "0 0 0 2px", letterSpacing: "0.04em" }}>
          {cloneName} <span style={{ animation: "blink 0.9s ease-in-out infinite" }}>●</span>
        </span>
        <div style={{
          ...
          {text
            ? <>{text}<span style={{ display: "inline-block", width: 2, height: 13, background: "rgba(255,255,255,0.50)", marginLeft: 2, verticalAlign: "middle", animation: "blink 0.65s ease-in-out infinite" }} /></>
            : <span style={{ display: "flex", gap: 3 }}>{[0,1,2].map(i => (
                <span key={i} style={{ width: 4, height: 4, borderRadius: "50%", background: "rgba(255,255,255,0.35)", display: "inline-block", animation: `blink 1.1s ease-in-out ${i * 0.18}s infinite` }} />
              ))}</span>
          }
        </div>
      </div>
...
function ConclusionCard({ conclusion }: { conclusion: string }) {
  return (
    <div style={{
      ...
      animation: "conclusion-in 350ms cubic-bezier(.4,0,.2,1) both",
    }}>
