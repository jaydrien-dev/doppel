...
function MermaidDiagram({ code }: { code: string }) {
  ...
  if (!svg) return (
    <div style={{ padding: "12px 16px", borderRadius: 10, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", fontSize: 12, color: "rgba(255,255,255,0.35)" }}>
      Rendering diagram…
    </div>
  );
    ...
    <div style={{ position: "relative", margin: "6px 0" }}>
      {language && (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "5px 12px", background: "rgba(255,255,255,0.06)", borderRadius: "10px 10px 0 0", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <span style={{ fontSize: 10, letterSpacing: "0.08em", textTransform: "uppercase", color: "rgba(255,255,255,0.35)" }}>{language}</span>
          <button
            onClick={handleCopy}
  ...
  return (
    <div className="md-prose">
      <style>{`
        ...
        .md-prose h1 { font-size: 18px; font-weight: 500; color: rgba(255,255,255,0.95); margin: 14px 0 8px; line-height: 1.3; }
        .md-prose h2 { font-size: 16px; font-weight: 500; color: rgba(255,255,255,0.90); margin: 12px 0 6px; line-height: 1.3; }
      ...
      <ReactMarkdown
        ...
        components={{
          ...
          pre({ children }) {
            // pre is handled by code component above
            return <>{children}</>;
          },
        }}
      >
